...
func (stack *Stack) Push(element *Element) {
	stack.elements = append(stack.elements[:stack.elementCont], element)
	stack.elementCont = stack.elementCont + 1
}

// Pop removes and returns a node from the stack in last to first order.
func (stack *Stack) Pop() *Element {
	if stack.elementCont == 0 {
		return nil
	}
	...
	stack.elementCont = len(stack.elements)
	return element
