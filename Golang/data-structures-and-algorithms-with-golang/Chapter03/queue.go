...
// New method initialises with Order with priority, quantity, product, custName
func (order *Order) New(priority int, quantity int, product string, custName string) {
		...
		*queue = append(*queue, order)
		...
		for i, addedOrder = range *queue {
			if order.priority > addedOrder.priority {
				*queue = append((*queue)[:], append(Queue{order}, (*queue)[i:]...)...)
				appended = true
				break
			}
		}
